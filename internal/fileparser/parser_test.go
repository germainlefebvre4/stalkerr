...
func TestParse(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "Embedded year-like title number with parenthesized release year",
			downloadPath: "/media/movies/Valensole 1965 (2025)/Valensole 1965 (2025).mkv",
			tmdbYear:     intPtr(2025),
			expected: &FileInfo{
				Extension:     ".mkv",
				...
				HasYearInPath: true,
				YearMismatch:  false,
				DetectedYear:  intPtr(2025),
				DetectedRes:   nil,
				IsValidFormat: true,
			},
		},
		{
			name:         "Multiple parenthesized years, last one selected",
			downloadPath: "/media/movies/Title (1984) (2021)/movie.mkv",
			tmdbYear:     intPtr(2021),
			expected: &FileInfo{
				Extension:     ".mkv",
