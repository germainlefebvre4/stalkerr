...
When a forced download completes, the system SHALL name the destination file so that it includes the occurrence's detected resolution as a distinguishing suffix (for example, a file named `Title (Year) [1080p].ext`), so that it cannot silently replace a sibling occurrence's file already present in the same movie/show destination folder. When the occurrence has no detected resolution, the system SHALL still apply a distinguishing marker unique to that download rather than omit the suffix. This naming rule SHALL apply only to files produced by a forced download; it SHALL NOT change the destination naming used by the existing automatic `radarr`/`sonarr` download commands.
...
- **WHEN** the existing automatic `radarr`/`sonarr` download commands complete a download
- **THEN** the destination file name SHALL follow the same convention as before this capability was introduced, without a resolution suffix
