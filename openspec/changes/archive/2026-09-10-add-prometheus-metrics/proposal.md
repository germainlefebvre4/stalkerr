...
- Add a new `job_runs` table (GORM-managed, alongside the existing `processing_logs` table) that persists start/end time, status, and succeeded/failed/skipped counts for every `resume-downloads` and `enrich-tvdb` invocation (both standalone CLI commands). This history is written unconditionally, independent of whether metrics exposition is enabled — it has standalone value as a durable run log for commands whose outcomes are today only logged and discarded.
- Add two nullable columns (`metadata_backfilled_count`, `metadata_backfill_errors_count`) to the existing `processing_logs` table, populated when a `process` run's internal rich-metadata backfill step (`internal/processor.BackfillRichMetadata`, invoked from within `process` — not a standalone command) completes. This follows the same pattern already used for that table's `movies_count`/`tmdb_matched_count`/etc. fields (see the `processing-run-statistics` capability).
- Add a config-gated `GET /metrics` endpoint, served on the existing admin port, in Prometheus text-exposition format, covering:
  - Run status, duration, and item counts (total processed / matched / skipped, both as a cumulative counter and as a last-run gauge) for every scheduled/batch action — sourced from `processing_logs` (`process`, `download`, `m3u-download`; `process`'s counts include the metadata-backfill outcome) and the new `job_runs` table (`resume-downloads`, `enrich-tvdb`) under one unified metric namespace and `action` label.
...
### New Capabilities
- `job-run-history`: Persists a `job_runs` row per invocation of `resume-downloads` and `enrich-tvdb`, recording status, duration, and succeeded/failed/skipped counts — independent of and useful without the metrics endpoint.
...
- `processing-run-statistics`: Adds a metadata-backfill outcome (updated count, error count) to what's persisted on a `process` run's `processing_logs` entry, following the same "persist on finalize, absent on pre-existing rows" pattern already established by that capability's other fields.
...
- **Modified code**: `internal/config` (new `MetricsConfig` struct), `cmd/resume_downloads.go`, `cmd/enrich_tvdb.go` (write to `job_runs` on start/finish), `internal/processor/processor.go` and `internal/models.ProcessingLog` (persist metadata-backfill counts), `internal/database/database.go` (register `JobRun` in `AutoMigrate`), `internal/external/tmdb` (expose circuit-breaker state), `config.yml.example`.
