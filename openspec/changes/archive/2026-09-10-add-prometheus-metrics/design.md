...
- `internal/models.ProcessingLog` (table `processing_logs`) already persists per-run stats for `process` only (written as `action="process_m3u"` from `internal/processor/processor.go`; see the `processing-run-statistics` capability). **Correction from an earlier draft of this document**: `download` (`cmd/download.go`) and `m3u-download` (`cmd/m3u.go`) do **not** currently write to `processing_logs` at all — like `resume-downloads`/`enrich-tvdb` before this change, they compute their own in-memory stats (`downloadStats`, per-source counters) but only log them, never persisting past process exit. Wiring those two commands to persist run history is out of scope for this change (see proposal.md's Impact/Out of scope); the metrics collector's `download`/`m3u-download` last-run status/duration and item-count metrics will therefore have no data to expose until that separate work happens — the metric families are still defined per the `prometheus-metrics` spec, they simply emit nothing for those two actions today. `resume-downloads` and `enrich-tvdb` are standalone CLI commands that compute equivalent stats in-memory (`ResumeStats`, `EnrichTVDBStats`) but only log them — nothing persists past process exit; this change adds `job_runs` to fix that gap for these two. Rich-metadata backfill (`BackfillStats`) is *not* a standalone command — `internal/processor.BackfillRichMetadata` is called from inside a `process` run (`processor.go:230`) and its counts are folded into that run's own in-memory `processor.Statistics`, also currently only logged, never persisted onto that run's `processing_logs` row.
...
### `job_runs`: one new generic table for the two standalone commands; `backfill-metadata` extends `processing_logs` instead
New GORM model `models.JobRun` (table `job_runs`): `Action`, `Status`, `StartedAt`, `CompletedAt *time.Time`, `SucceededCount`, `FailedCount`, `SkippedCount`, `ErrorMessage *string`, plus `CreatedAt`/`UpdatedAt`. `resume-downloads` and `enrich-tvdb` each create a row at start (`status=in_progress`) and finalize it at completion, mirroring `processing_logs`'s existing start/finalize lifecycle. Written unconditionally (see proposal.md), independent of `cfg.Metrics.Enabled`.

Rich-metadata backfill is *not* included in `job_runs` — it has no invocation of its own to attach a row to; it is a step inside a `process` run. Its counts are persisted as two new nullable columns directly on that run's existing `processing_logs` entry (`MetadataBackfilledCount *int`, `MetadataBackfillErrorsCount *int`), populated the same way `MoviesCount`/`TMDBMatchedCount`/etc. already are. This is a `processing-run-statistics` capability change, not a `job-run-history` one.

**Alternative considered**: add nullable `succeeded_count`/`failed_count`/`skipped_count` columns to `processing_logs` and reuse it for `resume-downloads`/`enrich-tvdb` too. Rejected for those two — `processing_logs`'s existing columns (`MoviesCount`, `TMDBMatchedCount`, ...) are specific to the M3U processing pipeline's semantics; forcing "resumed/failed/skipped" into that shape (or vice versa) would misrepresent one or the other, and unlike backfill, both are genuine standalone invocations that deserve their own row per run. A second table keeps each capability's persistence honest about what it actually measures, matching this repo's existing split between `processing-run-statistics` (persistence) and the capabilities that read it.
...
`action` (5 known values: `process`, `download`, `m3u-download`, `resume-downloads`, `enrich-tvdb`), `item_type`/`result` (a handful of known outcome kinds per action — `process`'s set includes `metadata_backfilled`/`metadata_backfill_errors` alongside its existing movies/tv-shows/tmdb-matched kinds), `status` (`download_info`'s existing status enum), `name` (currently only `"tmdb"`). No label is ever derived from user-controlled or unbounded data (e.g. no per-item or per-filename labels), avoiding Prometheus cardinality blowup.
...
- **[Risk]** `download` and `m3u-download` do not currently persist any per-run history to `processing_logs` (see the corrected Context bullet above) — their last-run status/duration and item-count metrics will be permanently absent (not merely sparse) until a separate change wires those two commands to persist run outcomes the same way `job_runs` now does for `resume-downloads`/`enrich-tvdb`. **Mitigation**: none built in; the metric families are still defined per spec so an operator adding that persistence later needs no metrics-side change, but this is a known gap in what's actually observable today.
...
- `models.ProcessingLog` gains two new nullable columns (`metadata_backfilled_count`, `metadata_backfill_errors_count`) — `AutoMigrate` adds them to the existing table on next startup; pre-existing rows read back as absent/null, consistent with `processing-run-statistics`'s existing "runs predating this capability report absent statistics" behavior for its other fields.
