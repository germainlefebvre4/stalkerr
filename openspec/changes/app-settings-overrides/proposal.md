...
- New Settings-drawer sections (frontend) to view and edit these settings, each field showing a two-state origin badge ("Interface" vs "Config" — file and env are not distinguished from each other) and, for the handful of settings still baked into long-lived server state at process start (TMDB client, downloader tuning, metrics listener), a "restart required" indicator rather than a false "applied immediately" claim.
...
- `frontend-app-settings-management`: Settings-drawer UI to view and edit `app-settings`-backed fields and manage M3U sources, including origin badges and restart-required indicators.
...
- `frontend-settings-drawer`: add new sections to the drawer's ordered section list (Radarr/Sonarr/TMDB/Jellyfin under an integrations-style section, Notifications, Downloads, and M3U sources), alongside the existing Apparence/Langue/Filtres/Préférences/Système/À propos.
...
- Frontend: `SettingsDrawer.tsx` plus new components/hooks per settings domain; locale updates in `src/locales/{en,fr}`.
